...
class ResultScreen extends StatelessWidget {
  ...
  final int? bloodSugar; // เพิ่มตัวแปร bloodPressure
  ...
  Future<int> _calculateTotalScore() async {
    ...
    if (quizSet.name == "ตรวจสุขภาพ") {
      try {
        ...
        // คำนวณคะแนนจากค่าความดันโลหิต
        if (bloodPressure != null) {
          if (bloodPressure! >= 120 && bloodPressure! < 140) {
            totalScore += 2; // ความดันระหว่าง 120 และ 140 เพิ่ม 2 คะแนน
          } else if (bloodPressure! >= 140) {
            totalScore += 4; // ความดัน 140 ขึ้นไป เพิ่ม 4 คะแนน
          }
        }

        if (bloodSugar != null) {
          if (bloodSugar! >= 0 && bloodSugar! < 99) {
            totalScore += 0; // ความดันระหว่าง 120 และ 140 เพิ่ม 2 คะแนน
          } else if (bloodSugar! >= 100) {
            totalScore += 5; // ความดัน 140 ขึ้นไป เพิ่ม 4 คะแนน
          }
        }
  ...
  void _showInfoDialog(BuildContext context) {
    showDialog(
      ...
      builder: (BuildContext context) {
        return AlertDialog(
          title: Text('รายละเอียดการเข้าตรวจ'),
          content: SingleChildScrollView(
            child: ListBody(
              children: [
                ...
                // เพิ่มข้อมูลอื่นๆ ได้ตามที่คุณต้องการ
              ],
            ),
          ),
          actions: [
